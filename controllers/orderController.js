...
const getLatLngFromAddress = async (addressString) => {
  try {
    ...
    logger.error('Geocode API Error: ' + err.message);
    return null;
  }
};
...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (userData?.email) {
      ...
      (async () => {
        try {
          ...
          if (!r.sent) logger.warn('Order placed email skipped: ' + r.error);
        } catch (e) {
          ...
          if (!r?.sent) logger.warn('Order placed email error: ' + e.message);
        }
      })();
    }
    ...
    logger.error('Create Order Error: ' + err.message, { stack: err.stack });
...
exports.getOrderById = async (req, res) => {
  try {
    ...
    logger.info('📦 Order items: ' + JSON.stringify(order.items, null, 2));
    ...
    logger.info('orderStatus: ' + formattedOrder.orderStatus);
    logger.info('statusHistory: ' + JSON.stringify(formattedOrder.statusHistory, null, 2));
    logger.info('paymentStatus: ' + formattedOrder.paymentStatus);
    logger.info('paymentMethod: ' + formattedOrder.paymentMethod);
    logger.info('totalAmount: ' + formattedOrder.totalAmount);
    logger.info('finalPayable: ' + formattedOrder.finalPayable);
    logger.info('customerName: ' + formattedOrder.customerName);
    logger.info('customerPhone: ' + formattedOrder.customerPhone);
    logger.info('deliveryAddress: ' + JSON.stringify(formattedOrder.deliveryAddress, null, 2));
    logger.info('assignedDeliveryBoy: ' + JSON.stringify(formattedOrder.assignedDeliveryBoy, null, 2));
    logger.info('items count: ' + formattedOrder.items?.length);
    logger.info('full order data: ' + JSON.stringify(formattedOrder, null, 2));
    ...
    logger.error('❌ Get Order Error: ' + err.message, { stack: err.stack });
    res.status(500).json({
      success: false,
...
exports.getAllCancelledOrders = async (req, res) => {
  try {
    ...
    logger.error('❌ Get Cancelled Orders Error: ' + err.message, { stack: err.stack });
    res.status(500).json({
      success: false,
...
exports.getAllOrders = async (req, res) => {
  try {
    ...
    logger.error('❌ Get All Orders Error: ' + err.message, { stack: err.stack });
    res.status(500).json({
      success: false,
...
exports.createOrderFromDirectBuy = async (req, res) => {
  try {
    ...
    logger.error('Create Direct Order Error: ' + err.message, { stack: err.stack });
    res.status(500).json({
...
exports.viewOrdersByShopAdmin = async (req, res) => {
  try {
    ...
    logger.error('❌ View Orders By Shop Admin Error: ' + err.message, { stack: err.stack });
    res.status(500).json({
      success: false,
  ...
  try {
    ...
    try {
      ...
      if (updatedOrder.userId) {
        ...
        logger.info(`📤 Emitted order status update to customer: ${updatedOrder.userId}`);
      } else {
        io.emit('orderStatusUpdated', statusUpdate);
      }
    } catch (err) {
      logger.error('Socket.IO emit failed: ' + err.message);
    }
    ...
    if (orderUser?.email) {
      sendOrderStatusUpdate(
        ...
        newStatus
      ).then((r) => { if (!r.sent) logger.warn('Order status email skipped: ' + r.error); })
        .catch((e) => logger.warn('Order status email error: ' + e.message));
    }
  ...
  try {
    ...
    if (updatedOrder.userId) {
      ...
      if (cancelUser?.email) {
        ...
        ).then((r) => { if (!r.sent) logger.warn('Order cancelled email skipped: ' + r.error); })
          .catch((e) => logger.warn('Order cancelled email error: ' + e.message));
      }
    }
    ...
    logger.error('Cancel Order Error: ' + error.message, { stack: error.stack });
    res.status(500).json({
...
exports.refundOrder = async (req, res) => {
  try {
    ...
    if (updatedOrder.userId) {
      ...
      if (refundUser?.email) {
        ...
        sendOrderStatusUpdate(
          ...
          'Refunded',
          refundMessage
        ).then((r) => { if (!r.sent) logger.warn('Order refunded email skipped: ' + r.error); })
          .catch((e) => logger.warn('Order refunded email error: ' + e.message));
      }
    }
    ...
    logger.error('Refund Order Error: ' + error.message, { stack: error.stack });
    res.status(500).json({
...
exports.deliveryBoyAcceptRejectOrder = async (req, res) => {
  try {
    const orderId = req.params.orderId;
    const { deliveryBoyId, action } = req.body;
    ...
    logger.error('Delivery Boy Accept/Reject Error: ' + error.message, { stack: error.stack });
    res.status(500).json({
...
exports.customerRaiseRefundRequest = async (req, res) => {
  try {
    ...
    logger.error('Customer Refund Request Error: ' + error.message, { stack: error.stack });
    res.status(500).json({
...
exports.autoAssignDeliveryBoyWithin5km = async (req, res) => {
  try {
    logger.info('🚀 API hit: autoAssignDeliveryBoyWithin5km');
    const orderId = req.params.orderId;
    ...
    logger.info(`📍 Raw shop location: "${shopLocationString}"`);

    const coords = shopLocationString.split(',').map(str => str.trim());

    if (coords.length !== 2) {
      logger.error(`❌ Invalid shop location format: ${shopLocationString}`);
      return res.status(400).json({
    ...
    if (isNaN(shopLatitude) || isNaN(shopLongitude)) {
      logger.error(`❌ Failed to parse coordinates: lat="${coords[0]}", lng="${coords[1]}"`);
      return res.status(400).json({
        message: 'Invalid shop location coordinates',
        success: false,
    ...
    if (shopLatitude < -90 || shopLatitude > 90) {
      logger.error(`❌ Invalid latitude: ${shopLatitude}`);
      return res.status(400).json({
        message: 'Invalid shop location coordinates',
        success: false,
    ...
    if (shopLongitude < -180 || shopLongitude > 180) {
      logger.error(`❌ Invalid longitude: ${shopLongitude}`);
      return res.status(400).json({
        message: 'Invalid shop location coordinates',
        success: false,
    ...
    logger.info(`✅ Valid shop coordinates: lat=${shopLatitude}, lng=${shopLongitude}`);
    ...
    logger.info(`📦 Found ${deliveryBoys.length} available delivery boys`);
    ...
    const deliveryBoysWithDistances = deliveryBoys.map(boy => {
      if (!boy.latitude || !boy.longitude || isNaN(boy.latitude) || isNaN(boy.longitude)) {
        logger.warn(`⚠️ Skipping delivery boy ${boy._id} - invalid coordinates`);
        return null;
      }
    ...
    for (const radius of radiusOptions) {
      nearbyDeliveryBoys = deliveryBoysWithDistances.filter(boy => boy.pickupDistance <= radius);
      if (nearbyDeliveryBoys.length > 0) {
        usedRadius = radius;
        logger.info(`✅ Found ${nearbyDeliveryBoys.length} delivery boys within ${radius} km`);
        break;
      }
    }
    ...
    try {
      io = getIO();
      logger.info('✅ Socket.IO instance retrieved');
      logger.info('📋 Currently connected users: ' + JSON.stringify(Object.keys(socketModule.getConnectedUsers())));
      ...
      logger.info(`   Delivery Earning: ${order.deliveryEarning} AED`);
      logger.info(`   Search Radius: ${usedRadius} km`);
      logger.info(`   Nearby Delivery Boys: ${nearbyDeliveryBoys.length}`);
      ...
      nearbyDeliveryBoys.forEach((boy, index) => {
        ...
        logger.info(`   Name: ${boy.name || 'N/A'}`);
        logger.info(`   Phone: ${boy.phone || 'N/A'}`);
        logger.info(`   Distance: ${boy.pickupDistance} km`);
        logger.info(`   Is Connected: ${isUserConnected(deliveryBoyId) ? '✅ YES' : '❌ NO'}`);

        if (isUserConnected(deliveryBoyId)) {
          logger.info(`   Socket ID: ${socketModule.getConnectedUsers()[deliveryBoyId]}`);
        }
        ...
        try {
          ...
          logger.info(`   ✅ Emission sent to room: ${deliveryBoyId}`);
          if (isUserConnected(deliveryBoyId)) successfulEmissions++;
          else failedEmissions++;
        } catch (emitError) {
          logger.error(`   ❌ Emission failed: ${emitError.message}`);
          failedEmissions++;
        }
      });

      logger.info('\n📊 Emission Summary:');
      logger.info(`   ✅ Successful: ${successfulEmissions}`);
      logger.info(`   ❌ Failed/Uncertain: ${failedEmissions}`);
      logger.info(`   📱 Total attempted: ${nearbyDeliveryBoys.length}`);

    } catch (err) {
      logger.error('❌ Socket.IO error: ' + err.message, { stack: err.stack });
    }

    logger.info('🔔 ========== SOCKET EMISSION END ==========\n');
    ...
    logger.error('❌ Auto Assign Delivery Boy Error: ' + error.message, { stack: error.stack });
...
exports.getNearbyAssignedOrders = async (req, res) => {
  try {
    ...
    logger.info(`📍 Delivery Boy Location: ${boyLat}, ${boyLng}`);
    ...
    for (const order of assignedOrders) {
      ...
      if (!shopLocation) {
        logger.warn(`⚠️ Shop location not found for order ${order._id}`);
        continue;
      }

      const [shopLat, shopLng] = shopLocation.split(',').map(Number);
      logger.info(`📍 Shop Location: ${shopLat}, ${shopLng}`);
      logger.info(`📍 Customer Location: ${order.deliveryAddress?.latitude}, ${order.deliveryAddress?.longitude}`);
      ...
      logger.info(`🚗 Pickup Distance: ${pickupDistance.toFixed(2)} km`);

      if (pickupDistance > RANGE_KM) {
        logger.info(`❌ Order ${order._id} too far: ${pickupDistance.toFixed(2)} km`);
        continue;
      }
      ...
      try {
        ...
        if (pickupElement?.status === 'OK' && pickupElement?.duration) {
          ...
          logger.info(`⚠️ Pickup time API returned: ${pickupElement?.status}`);
          pickupTime = `${Math.ceil((pickupDistance / 30) * 60)} mins`;
        }
      } catch (err) {
        logger.info(`❌ Pickup time API error: ${err.message}`);
        pickupTime = `${Math.ceil((pickupDistance / 30) * 60)} mins`;
      }
      ...
      if (
        ...
        logger.info(`🚚 Drop Distance: ${dropDistance.toFixed(2)} km`);

        const fallbackDropTime = `${Math.ceil((dropDistance / 30) * 60)} mins`;
        dropTime = fallbackDropTime;
        ...
        if (dropDistance < 100) {
          try {
            ...
            if (dropElement?.status === 'OK' && dropElement?.duration) {
              dropTime = dropElement.duration.text;
              logger.info(`✅ Drop time from API: ${dropTime}`);
            } else {
              logger.info(`⚠️ Drop time API returned: ${dropElement?.status}, using fallback: ${fallbackDropTime}`);
            }
          } catch (err) {
            logger.info(`❌ Drop time API error: ${err.message}, using fallback: ${fallbackDropTime}`);
          }
        } else {
          logger.info(`⚠️ Drop distance too large (${dropDistance.toFixed(2)} km), using fallback: ${fallbackDropTime}`);
        }
      } else {
        logger.info(`❌ Invalid coordinates - Shop: (${shopLat}, ${shopLng}), Customer: (${customerLat}, ${customerLng})`);
      }
    ...
    logger.info(`✅ Found ${formattedNearbyOrders.length} nearby assigned orders`);
    ...
    logger.error('Get Nearby Assigned Orders Error: ' + error.message, { stack: error.stack });
    return res.status(500).json({
...
async function getInvoicePdfBufferForMasterOrder(orders, options) {
  if (!orders || orders.length === 0) return getInvoicePdfBuffer(orders[0]);
...
// Generate Invoice PDF (download response).
exports.generateInvoice = async (req, res) => {
  try {
    ...
    logger.error('Invoice generation failed: ' + err.message, { stack: err.stack });
...
exports.sendInvoiceByEmail = async (req, res) => {
  try {
    ...
    logger.error('Send invoice by email failed: ' + err.message, { stack: err.stack });
    res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.getCancellationReasons = async (req, res) => {
  try {
    ...
    logger.error('Get Cancellation Reasons Error: ' + error.message, { stack: error.stack });
    res.status(500).json({
      success: false,
